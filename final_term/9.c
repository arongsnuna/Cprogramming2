//none option: print the word alphabetically
//-r option: print the word in the reverse alphabetically
#include <stdio.h>
#include <string.h>
...
{	
	...
	if (strcmp(argv[0], "-r") ==0){ //if there is option -r
			for (i=argc-1; i>1; i--)
				printf("%s ", argv[i]);
			//print the argv in reverse order
	}
	else { //if there is not option -r
		for (i = 1; i < argc; i++)
			printf("%s ", argv[i]);
		//print the argv
	}
